...
export default function 개수입력Modal() {
  ...
  return (
    <>
      <button
        className="group rounded-lg border border-transparent px-5 py-4 transition sm:text-left hover:border-gray-300 hover:bg-gray-100 hover:dark:border-neutral-700 hover:dark:bg-neutral-800/30"
      ...
      <Modal open={open} onClose={() => setOpen(false)} showCloseButton showDragButton>
        <form className=" grid gap-3 px-4 pt-5 pb-4 rounded-lg bg-white shadow-lg" onSubmit={handleSubmit}>
          <h3 className="px-2 font-semibold text-lg">문제 개수를 입력해주세요</h3>
          <input className="w-full px-2 border" required min={0} max={50} name="문제개수" type="number" />
          <button
